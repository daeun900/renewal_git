...
if($NotPDF=="Y") {//여기로 옴
	...
	$pdf->AddUHCFont('바탕', 'Batang');
		...
		$contentsNameArr2 = explode("_", $contentsName); //차시명 길어져서 : 기준으로 자르기
		if(count($contentsNameArr2) > 1){
		    $contentsName1 = $contentsNameArr2[0];
		    $contentsName2 = $contentsNameArr2[1];
		    $pdf->WriteHTML("                훈련과정 : ". $contentsName1." _ ");
		    $pdf->Ln(7);
		...
		if($LectureCode=="AAAA1" || $LectureCode=="AAAA1") {
			...
			if($LectureCode=="AAAA1") {
				...
				$pdf->Image('../../images/common/company_stamp.png',128,247,20,21,'','');
			}
			...
			//$pdf->Image('../images/company_stamp.png',148,238,20,21,'','');
                        $pdf->Image('../../images/common/company_stamp3.png',138,236,20,21,'','');
		}
