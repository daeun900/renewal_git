				...
				*/
				...
				if(ConnStatus == "101") {
					if(OtpStatus == "101" || OtpStatus == "103") {
						...
						location.href="/player/player_otp_init.php?Chapter_Number="+Chapter_Number+"&LectureCode="+LectureCode+"&Study_Seq="+Study_Seq+"&Chapter_Seq="+Chapter_Seq+"&Contents_idx="+Contents_idx+"&mode="+mode;
					}
					else {
		...
		$.ajax({
			...
			success : function(xml) {					
				if($(xml).find("RetVal").text() == "101") {
					...
					$.post("/player/player_captcha_session.php",
					{
